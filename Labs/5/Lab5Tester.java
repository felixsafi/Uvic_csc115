...
public class Lab5Tester {
	...
	public static boolean doBracketsMatch(String str) {
		...
		if (str.length()>0){
			...
			int tracker = 0;
			while(!stk.isEmpty()&&tracker>=0){
				char character=stk.pop();
				if(character=='(')tracker++;
				if(character==')')tracker--;
			}
	...
	public static void testStackUseFunctions() {
		...
		matched = doBracketsMatch("((abc)");
		displayResults(!matched, "double open, one close");

		matched = doBracketsMatch("(())");
		displayResults(matched, "true only brackets");
