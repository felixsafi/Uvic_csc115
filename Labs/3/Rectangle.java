public class Rectangle implements Shape {
	...
	public double area(){
		return (this.length*this.height*1.0);
	}
	public double perimeter(){
		return (this.length*2+this.height*2)*1.0;
	}



	public boolean contains(Point pt){
        int ptX = Math.abs(pt.getX());
        int ptY = Math.abs(pt.getY());
        
        int positionX = Math.abs(position.getX());
        int positionY = Math.abs(position.getY());
        
        if ((ptY<=positionY+this.height&&ptY>=positionY)&&(ptX<=positionX+this.length&&ptX>=positionX)) {
            return true;
		}        
		return false;
	}
